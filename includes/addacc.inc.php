...
if (isset($_POST['submit'])) {
    ...
    if ($an="" || $dsc="" || $igu="") {
     ...
     if (!preg_match("/^[a-zA-Z]*$/", $first) || !preg_match("/^[a-zA-Z]*$/", $last)) {
        ...
        $dscc = mysqli_real_escape_string($db, $_POST['desc']);
        $iguu = mysqli_real_escape_string($db, $_POST['igusername']);
        $owner = mysqli_real_escape_string($db, $_SESSION['u_id']);
        //insert the user into the db
        $sql = "INSERT INTO accounts (account_name,account_description,list_points,account_redirect,account_img,account_owner,account_status) VALUES ('$acnn','$dscc','','$iguu','','$owner','Free');";
